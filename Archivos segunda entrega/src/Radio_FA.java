...
public class Radio_FA implements iradio{
	...
	private boolean estado; //Este atributo servirá para saber si el radio esta encendido o apagado
	private boolean cambiarFrecuencia; //Este atributo servira para saber si la emisora esta en AM o en FM
	private float [] guardadas = new float [12]; // Aqui se guardaran las 12 emisoras que se deseen

	public Radio_FA(){
            cambiarFrecuencia=true;
            estado = false;
	...
	*/
	...
	public float Switch(){ //Método para cambiar AM/FM
		if(cambiarFrecuencia){
			cambiarFrecuencia = false; //true significa FM
		}else{
			cambiarFrecuencia= true; // false significa AM
		}
		float a = 0;
                return a;
	}
